/* Hero background: abstract topology — layered nodes wired top-to-bottom,
   slow signal pulses drifting along the edges (ported from the TF-ART sibling
   site). Colors resolve from tokens.css at init — never hard-code hex here.
   Perf contract: single rAF, ~30 paths, dash-offset writes only, IO pause. */
export function initHeroTopo() {
  ...
  // Layered topology: 5 rows echoing perception → reasoning → action
  const rows = [
    { y: 90,  n: 6 },
    { y: 280, n: 9 },
    { y: 470, n: 5 },
